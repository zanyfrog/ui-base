...
export class UibLayoutPreview extends BaseHTMLElement {
  ...
  private renderNode(node: LayoutNode): string {
    if (node.kind === 'text') {
      return `<button part="button" type="button" class="text ${node.id === this.selectedNodeId ? 'selected' : ''}" data-node-id="${attr(node.id)}">${escapeHtml(node.text || node.label)}</button>`;
    }
    if (node.kind === 'dynamic') {
      return `<button part="button" type="button" class="dynamic ${node.id === this.selectedNodeId ? 'selected' : ''}" data-node-id="${attr(node.id)}">${escapeHtml(node.label)}</button>`;
    }
    const tag = node.tagName || node.kind;
    return `
      <button part="button" type="button" class="box ${node.id === this.selectedNodeId ? 'selected' : ''} ${node.editable ? '' : 'readonly'}" data-node-id="${attr(node.id)}">
...
defineLayoutElement('uib-layout-preview', UibLayoutPreview);
