import { BaseHTMLElement, defineLayoutElement, escapeHtml } from './dom-utils.js';

export class UibLayoutDiff extends BaseHTMLElement {
  private internalDiffText = '';
  private internalWarnings: string[] = [];
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="wrap" part="wrap">
        ${this.warnings.length ? `<div class="warnings" part="warnings">${this.warnings.map((warning) => `<div>${escapeHtml(warning)}</div>`).join('')}</div>` : ''}
...
defineLayoutElement('uib-layout-diff', UibLayoutDiff);
