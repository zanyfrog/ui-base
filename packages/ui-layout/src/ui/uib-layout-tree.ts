...
export class UibLayoutTree extends BaseHTMLElement {
  private internalNodes: LayoutNode[] = [];
  ...
  private bind() {
    this.shadowRoot?.querySelectorAll<HTMLButtonElement>('[data-node-id]').forEach((button) => {
  ...
  private renderNode(node: LayoutNode, depth = 0): string {
    const readonly = !node.editable;
    const selected = node.id === this.selectedNodeId;
    const summary = node.classList.length ? `.${node.classList.slice(0, 3).join('.')}` : '';
    return `
      <li>
        <button type="button" draggable="${node.editable && node.kind !== 'dynamic' && node.kind !== 'fragment' ? 'true' : 'false'}" data-node-id="${attr(node.id)}" class="${selected ? 'selected' : ''}" style="--depth:${depth}">
          <span class="kind">${escapeHtml(node.kind)}</span>
          <span class="label">${escapeHtml(node.tagName || node.label)}${escapeHtml(summary)}</span>
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host{display:block;min-width:0;color:var(--uib-layout-ink,#172033);font:14px/1.4 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
        ul{list-style:none;margin:0;padding:0}
        button{width:100%;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:.45rem;padding:.45rem .5rem .45rem calc(.5rem + var(--depth,0) * .85rem);border:0;border-radius:6px;background:transparent;color:inherit;text-align:left;font:inherit;cursor:pointer}
        button:hover{background:#eef4fb}
        button.selected{background:#dfefff;box-shadow:inset 3px 0 0 #245ea8}
        button.drop-before{box-shadow:inset 0 3px 0 #245ea8}
        button.drop-after{box-shadow:inset 0 -3px 0 #245ea8}
        .kind{padding:.08rem .3rem;border:1px solid #c9d5e5;border-radius:999px;color:#47627f;font-size:.72rem;text-transform:uppercase;white-space:nowrap}
        .label{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .readonly{color:#7b8797;font-size:.72rem;text-transform:uppercase}
      </style>
      <ul>${this.nodes.length ? this.nodes.map((node) => this.renderNode(node)).join('') : '<li class="empty">No layout nodes.</li>'}</ul>
    `;
    this.bind();
  }
}

defineLayoutElement('uib-layout-tree', UibLayoutTree);
