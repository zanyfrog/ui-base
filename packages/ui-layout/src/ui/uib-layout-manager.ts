...
import { applyLayoutOperationsToSource, type PatchResult } from '../writer/apply-layout-operations.js';
...
export class UibLayoutManager extends BaseHTMLElement {
  ...
  private internalSourceText = '';
  private internalFileName = '';
  private internalFileAttributes: Record<string, unknown> = {};
  private internalAnalysis: LayoutAnalysis | null = null;
  private internalOperations: LayoutOperation[] = [];
  private internalSelectedNodeId = '';
  private internalPatchResult: PatchResult | null = null;
  ...
  set fileAttributes(value: Record<string, unknown>) {
    this.internalFileAttributes = value && typeof value === 'object' ? value : {};
    this.analyze();
  }

  get fileAttributes(): Record<string, unknown> {
    return this.internalFileAttributes;
  }

  set analysis(value: LayoutAnalysis | null) {
    this.internalAnalysis = value;
    this.render();
  }

  get analysis(): LayoutAnalysis | null {
    return this.internalAnalysis;
  }

  set operations(value: LayoutOperation[]) {
  ...
  get operations(): LayoutOperation[] {
  ...
  analyze() {
    const input: AnalyzeSourceInput = {
  ...
  private selectedNode(): LayoutNode | null {
    if (!this.analysis) return null;
    return flattenLayoutNodes(this.analysis.rootNodes).get(this.internalSelectedNodeId) ?? null;
  }

  private firstSelectableNode(nodes: LayoutNode[]): LayoutNode | null {
  ...
  private addOperation(operation: LayoutOperation) {
  ...
  private createPatchResult(): PatchResult {
  ...
  private bind() {
    ...
    this.shadowRoot.addEventListener('uib-layout-operation-added', (event) => {
      const detail = (event as CustomEvent<{ operation?: LayoutOperation }>).detail;
    ...
    this.shadowRoot.addEventListener('uib-layout-node-move-requested', (event) => {
      const detail = (event as CustomEvent<{ nodeId?: string; targetNodeId?: string; placement?: 'before' | 'after' }>).detail;
      event.stopPropagation();
    ...
    this.shadowRoot.addEventListener('uib-layout-node-keyboard-move-requested', (event) => {
      const detail = (event as CustomEvent<{ nodeId?: string; direction?: 'up' | 'down' }>).detail;
      event.stopPropagation();
  ...
  private moveOperationForDrop(detail: { nodeId?: string; targetNodeId?: string; placement?: 'before' | 'after' }): LayoutOperation | null {
  ...
  private moveOperationForKeyboard(detail: { nodeId?: string; direction?: 'up' | 'down' }): LayoutOperation | null {
  ...
  private indexNodes(nodes: LayoutNode[], parent: LayoutNode | null = null, map = new Map<string, { node: LayoutNode; parent: LayoutNode | null }>()): Map<string, { node: LayoutNode; parent: LayoutNode | null }> {
    for (const node of nodes) {
  ...
  private isMovable(node: LayoutNode): boolean {
  ...
  private syncChildren() {
    ...
    const tree = this.shadowRoot.querySelector('uib-layout-tree') as HTMLElement & { nodes: LayoutNode[]; selectedNodeId: string };
    const preview = this.shadowRoot.querySelector('uib-layout-preview') as HTMLElement & { nodes: LayoutNode[]; selectedNodeId: string };
    const properties = this.shadowRoot.querySelector('uib-layout-properties') as HTMLElement & { node: LayoutNode | null; readonly: boolean };
    const diff = this.shadowRoot.querySelector('uib-layout-diff') as HTMLElement & { diffText: string; warnings: string[] };
    if (tree) {
...
declare global {
  interface HTMLElementTagNameMap {
    'uib-layout-manager': UibLayoutManager;
  }
}
