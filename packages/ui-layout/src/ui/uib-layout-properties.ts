...
export class UibLayoutProperties extends BaseHTMLElement {
  private internalNode: LayoutNode | null = null;
  ...
  set node(value: LayoutNode | null) {
    this.internalNode = value;
    this.render();
  }

  get node(): LayoutNode | null {
  ...
  private bind() {
    ...
    this.shadowRoot?.querySelector<HTMLButtonElement>('[data-apply-classes]')?.addEventListener('click', () => {
      const value = this.shadowRoot?.querySelector<HTMLInputElement>('[data-classes]')?.value ?? '';
    ...
    this.shadowRoot?.querySelector<HTMLButtonElement>('[data-apply-attribute]')?.addEventListener('click', () => {
      const name = this.shadowRoot?.querySelector<HTMLInputElement>('[data-attribute-name]')?.value ?? '';
      const value = this.shadowRoot?.querySelector<HTMLInputElement>('[data-attribute-value]')?.value ?? '';
      if (name.trim()) this.emitOperations([this.createOperation(node.id, 'set-attribute', { name: name.trim(), value })]);
    });

    this.shadowRoot?.querySelectorAll<HTMLButtonElement>('[data-update-attribute]').forEach((button) => {
      button.addEventListener('click', () => {
        const originalName = button.dataset.updateAttribute ?? '';
        const row = button.closest<HTMLElement>('[data-attribute-row]');
        const name = row?.querySelector<HTMLInputElement>('[data-existing-attribute-name]')?.value.trim() ?? originalName;
        const value = row?.querySelector<HTMLInputElement>('[data-existing-attribute-value]')?.value ?? '';
        if (!name) return;
        const operations: LayoutOperation[] = [];
    ...
    this.shadowRoot?.querySelectorAll<HTMLButtonElement>('[data-remove-attribute]').forEach((button) => {
      button.addEventListener('click', () => {
    ...
    this.shadowRoot?.querySelector<HTMLButtonElement>('[data-apply-text]')?.addEventListener('click', () => {
      const value = this.shadowRoot?.querySelector<HTMLTextAreaElement>('[data-text]')?.value ?? '';
      this.emitOperations([this.createOperation(node.id, 'set-text', { value })]);
    });
  }

  private createOperation(nodeId: string, type: LayoutOperation['type'], extra: Record<string, unknown>): LayoutOperation {
    return {
  ...
  private emitOperations(operations: LayoutOperation[]) {
      ...
      <div class="panel" part="panel">
        ...
        ${node ? `
          ...
          ${node.kind === 'element' || node.kind === 'component' ? `
            ...
            <button part="button" type="button" data-apply-classes ${locked ? 'disabled' : ''}>Apply classes</button>
            ...
            <button part="button" type="button" data-apply-attribute ${locked ? 'disabled' : ''}>Set attribute</button>
            <ul class="attrs">${Object.entries(node.attributes).map(([name, value]) => `
              <li class="attr-row" data-attribute-row>
                ...
                <button part="button" class="secondary" type="button" data-update-attribute="${attr(name)}" ${locked ? 'disabled' : ''}>Update</button>
                <button part="button danger-button" class="danger" type="button" data-remove-attribute="${attr(name)}" ${locked ? 'disabled' : ''}>Remove</button>
              </li>
            `).join('')}</ul>
          ` : ''}
          ${node.kind === 'text' ? `
            ...
            <button part="button" type="button" data-apply-text ${locked ? 'disabled' : ''}>Apply text</button>
          ` : ''}
...
defineLayoutElement('uib-layout-properties', UibLayoutProperties);
