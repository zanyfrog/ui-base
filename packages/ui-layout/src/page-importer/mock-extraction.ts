import type { PageExtractionResult } from '../model/page-import-artifact.js';

export function createMockPageExtractionResult(sourceUrl = 'https://example.local/customer-intake'): PageExtractionResult {
...
function field(id: string, label: string, inputType: string, name: string, elementId: string, value: string, required: boolean, order: number, placeholder = '', componentTag = 'uib-forms-textbox') {
  return {
    id,
    kind: 'field' as const,
    label,
...
function treeItems(ids: string[]) {
  return ids.map((id) => ({
    id: `node_${id}`,
    label: id.replace(/^item_/, '').replaceAll('_', ' '),
    kind: id.includes('field') ? 'field' as const : 'group' as const,
    itemId: id,
    children: [],
  }));
}
