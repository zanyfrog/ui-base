import { createPageImportArtifact,                                                                                                                                                   } from '../model/page-import-artifact.js';
...
export class UibPageImporter extends BaseHTMLElement {
  ...
  disconnectedCallback() {
    ...
    this.render();
          ...
          updateItem(itemId        , property                                              , value        ) {
    if (!this.artifact) return;
    this.artifact = {
      ...this.artifact,
      metadata: { ...this.artifact.metadata, updatedAt: new Date().toISOString() },
    ...
    this.render();
          ...
          updateDatabase(itemId        , property                                      , value        ) {
    if (!this.artifact) return;
    this.artifact = {
      ...this.artifact,
      metadata: { ...this.artifact.metadata, updatedAt: new Date().toISOString() },
    ...
    root.querySelector                  ('[data-show-hidden]')?.addEventListener('change', (event) => {
      this.showHidden = (event.currentTarget                    ).checked;
      this.render();
    });
    ...
    root.querySelectorAll                                        ('[data-item-field]').forEach((field) => {
      field.addEventListener('change', () => this.updateItem(field.dataset.itemId || '', field.dataset.itemField                                                , field.value));
    });
    root.querySelectorAll                  ('[data-db-field]').forEach((field) => {
      field.addEventListener('change', () => this.updateDatabase(field.dataset.itemId || '', field.dataset.dbField                                        , field.value));
    });
    root.querySelectorAll                   ('[data-toggle-hidden]').forEach((button) => {
      button.addEventListener('click', () => this.toggleHidden(button.dataset.itemId || ''));
    });
    root.querySelectorAll                   ('[data-move-item]').forEach((button) => {
      button.addEventListener('click', () => this.moveItem(button.dataset.itemId || '', button.dataset.moveItem                 ));
    });
...
function displayPageName(artifact                    )         {
...
function appendArtifactLog(artifact                    , entry                                    )                     {
  return {
    ...artifact,
...
async function writeDraft(artifact                    )                {
  const db = await openDraftDb();
  return new Promise((resolve, reject) => {
