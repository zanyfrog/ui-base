...
export function parseHtmlFragment(rawText        , context                          )               {
  const roots               = [];
  const stack               = [];
  let cursor = 0;
...
function appendNode(node            , roots              , stack              )       {
  const parent = stack[stack.length - 1];
  if (parent) parent.children.push(node);
  else roots.push(node);
}

function elementNode(tagName        , attributes                        , start        , end        , context                          )             {
  ...
  const allowedOperations                        = editable
...
function textNode(text        , start        , end        , context                          )             {
  const absoluteStart = context.sourceOffset + start;
  const position = lineColumnForIndex(context.sourceText, absoluteStart);
  return {
...
function dynamicNode(expression        , start        , end        , context                          )             {
  const absoluteStart = context.sourceOffset + start;
  const position = lineColumnForIndex(context.sourceText, absoluteStart);
  return {
...
function nextSpecialIndex(rawText        , from        )         {
...
function findTagEnd(rawText        , start        )         {
  let quote = '';
...
function findDynamicExpressionEnd(rawText        , start        )         {
  let depth = 0;
  let quote = '';
...
function parseOpeningTag(openingTag        )                                                                                       {
