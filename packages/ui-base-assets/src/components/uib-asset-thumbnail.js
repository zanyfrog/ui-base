    ...
    this.shadowRoot.innerHTML = `
      <style>${baseAssetStyles}
        .thumb {
          ...
          background:
            linear-gradient(135deg, rgba(23, 74, 139, 0.08), rgba(244, 189, 70, 0.12)),
            var(--uib-assets-surface-soft);
        }
        img { width: 100%; height: 100%; min-height: 7rem; object-fit: cover; }
        .fallback { display: grid; gap: 0.35rem; place-items: center; padding: 0.85rem; color: var(--uib-assets-muted); text-align: center; }
        .file-icon {
          position: relative;
          width: 3.7rem;
          height: 4.4rem;
        ...
        .file-icon::before {
          ...
          top: -1px;
          right: -1px;
          border-top: 1.05rem solid var(--uib-assets-surface-soft);
        ...
        .file-icon[data-kind="pdf"] { color: #b4232a; }
        .file-icon[data-kind="word"] { color: #174a8b; }
        .file-icon[data-kind="excel"] { color: #2e7d32; }
        .file-icon[data-kind="powerpoint"] { color: #b85c00; }
        .file-icon[data-kind="video"] { color: #6c3dbb; }
        .file-icon[data-kind="audio"] { color: #7a4f00; }
        .file-label { font-size: 0.75rem; font-weight: 800; }
      </style>
      <div class="thumb" role="img" aria-label="${escapeHtml(asset.name)} thumbnail">
