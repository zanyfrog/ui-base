...
export class UibAssetPreview extends BaseHTMLElement {
  ...
  `<code>` +
  (escapeHtml(JSON.stringify(asset.raw?.raw || asset.raw || { assetId: asset.id, key: asset.key }, null, 2))) +
  `</code>` +
  `</pre>`
);
    }
  ...
  `</iframe>`
    ...
    if (asset.url && asset.fileType === 'external_url') {
      return (
  `<p>` +
  `<a class="asset-button primary" href="` +
  (escapeHtml(asset.url)) +
  `" target="_blank" rel="noreferrer"> Open external asset ` +
  `</a>` +
  `</p>`
);
    }
  ...
  `<style>` +
  (baseAssetStyles) +
  ` .preview { display: grid; gap: 1rem; padding: 1rem; } .preview-media { overflow: hidden; min-height: 220px; display: grid; place-items: center; border: 1px solid var(--uib-assets-border); border-radius: 1rem; background: var(--uib-assets-surface-soft); } img { width: 100%; height: min(52vh, 420px); object-fit: contain; } iframe { width: 100%; min-height: 420px; border: 0; background: white; } .meta { display: flex; flex-wrap: wrap; gap: 0.45rem; } ` +
  `</style>` +
