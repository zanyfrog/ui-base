...
export class UibAssetFilterBar extends BaseHTMLElement {
  ...
  `<style>` +
  (baseAssetStyles) +
  ` .filters { display: grid; grid-template-columns: repeat(5, minmax(130px, 1fr)); gap: 0.7rem; } @media (max-width: 980px) { .filters { grid-template-columns: repeat(2, minmax(0, 1fr)); } } @media (max-width: 560px) { .filters { grid-template-columns: 1fr; } } ` +
  `</style>` +
  `<div class="filters" aria-label="Asset filters">` +
  `<label>` +
  `Type ` +
  `<select name="fileType">` +
  (option('all', 'All types', filters.fileType)) +
  ...
  (this._categories.map((category) => option(category.key, category.name || category.key, filters.category)).join('')) +
  `</select>` +
  `</label>` +
  `<label>` +
  `Status ` +
