  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>${baseCalendarStyles}
        ...
        .day-date { margin: 0; font-size: clamp(2rem, 7vw, 4.5rem); line-height: 1; font-weight: 900; }
        .day-name { margin: 0.35rem 0 1rem; font-size: 1.1rem; color: var(--uib-color-muted,#53657f); }
      </style>
      <section class="calendar-card" part="card" aria-label="Day view">
        <div class="calendar-header" part="header">
          <div>
            <h2 class="calendar-title" part="title">Day View</h2>
            <p class="calendar-subtitle" part="subtitle">A single date controlled by the parent.</p>
          </div>
        </div>
        <div class="day-panel" part="body">
          <p class="day-date" part="day-date">${date.getDate()}</p>
          <p class="day-name" part="day-name">${escapeHtml(formatLong(date))}</p>
          <button type="button" part="action" data-date="${escapeHtml(date.toISOString())}">Select this day</button>
        </div>
      </section>
    `;
