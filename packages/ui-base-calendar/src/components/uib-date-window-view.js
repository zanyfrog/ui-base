...
export class UibDateWindowView extends BaseHTMLElement {
  ...
  ` ` +
  (baseCalendarStyles) +
  ` .window-grid { grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); } .weekday { display: block; color: var(--uib-color-muted,#53657f); font-size: 0.72rem; font-weight: 850; text-transform: uppercase; } .day-number { display: block; margin-top: 0.25rem; font-size: 1.15rem; } ` +
  `</style>` +
  ...
  ` ` +
  (escapeHtml(count)) +
  ` consecutive dates starting ` +
  (escapeHtml(toIsoDate(start))) +
  ` ` +
  `</p>` +
  `</div>` +
  `</div>` +
