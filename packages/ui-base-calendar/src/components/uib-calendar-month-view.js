  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>${baseCalendarStyles}
        ...
        .day-label { padding: 0.3rem 0; color: var(--uib-color-muted,#53657f); text-align: center; font-size: 0.75rem; font-weight: 850; text-transform: uppercase; }
        .empty { min-height: 3.2rem; border-radius: 12px; background: #f6f8fc; }
        @media (max-width: 560px) { .month-grid { gap: 0.35rem; } .date-button, .empty { min-height: 2.7rem; } }
      </style>
      <section class="calendar-card" part="card" aria-label="Month view">
        <div class="calendar-header" part="header">
          <div>
            <h2 class="calendar-title" part="title">${MONTH_NAMES[month - 1]} ${year}</h2>
            <p class="calendar-subtitle" part="subtitle">Parent controls year, month, and selected date.</p>
          </div>
        </div>
