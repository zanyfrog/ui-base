  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>${baseCalendarStyles}
        ...
        .month-button { min-height: 5rem; }
        .month-button[aria-current="date"] { background: var(--uib-color-primary,#174a8b); color: var(--uib-color-primary-contrast,#fff); border-color: var(--uib-color-primary,#174a8b); }
        @media (max-width: 700px) { .year-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
      </style>
      <section class="calendar-card" part="card" aria-label="Year view">
        <div class="calendar-header" part="header">
          <div>
            <h2 class="calendar-title" part="title">${year}</h2>
            <p class="calendar-subtitle" part="subtitle">Select a month and let the parent decide the next view.</p>
          </div>
        </div>
